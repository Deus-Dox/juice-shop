...
exports.serveCodeSnippet =
  ...
  async (
    ...
    try {
      ...
      res
        .status(statusCode)
        .json({ status: "error", error: utils.getErrorMessage(error) });
    }
  };
...
exports.checkVulnLines =
  ...
  async (
    ...
    if (fs.existsSync("./data/static/codefixes/" + safeKey + ".info.yml")) {
      const codingChallengeInfos = yaml.load(
        fs.readFileSync(
          "./data/static/codefixes/" + safeKey + ".info.yml",
          "utf8",
        ),
      );
      if (codingChallengeInfos?.hints) {
        if (
          accuracy.getFindItAttempts(key) > codingChallengeInfos.hints.length
        ) {
          ...
          const nextHint =
            codingChallengeInfos.hints[accuracy.getFindItAttempts(key) - 1]; // -1 prevents after first attempt
